...
class Dataset:
    def __init__(self):
        self.list_images2video_dirs = ["MOT20/train/MOT20-01/"]  # , "MOT20/train/MOT20-05/", "MOT20/test/MOT20-06/"]

    def create_videos(self):
        for movi in self.list_images2video_dirs:
            print(movi)
    ...
    def save_labels_from_sample(self, sample):
        # for sample in dataset[:1]:
        # Match image filename root
        video_stem = os.path.splitext(os.path.basename(sample.filepath))[0]
        print(video_stem)
        os.makedirs(os.path.join(root_folder, f"train_mot20/{video_stem}/labels"))

        for i in range(len(sample.frames)):
            frame_number = i + 1
            f = sample.frames[frame_number]
            output_file = open(
                os.path.join(root_folder, f"train_mot20/{video_stem}/labels/{frame_number:06d}.txt"), "w"
            )
    ...
    def add_ground_truth(self, view, gt_path="MOT20/train/MOT20-03/gt/gt.txt"):
        # dataset = fo.load_dataset("mot20")
        # view = dataset.skip(2)
        # frames = view.to_frames(sample_frames=True)
        df = pd.read_table(
            os.path.join(root_folder, gt_path),
            header=None,
            sep=",",
        )
        # view = dataset.last()
    ...
    def add_new_sample(self, path="movi_MOT20-01.mp4", gt_path="MOT20/train/MOT20-01/gt/gt.txt"):
        dataset = fo.load_dataset("mot20")
        sample = fo.Sample(filepath=os.path.join(mv_dir, path))
        dataset.add_sample(sample)
        view = dataset.skip(3)
        frames = view.to_frames(sample_frames=True)
        Dataset().add_ground_truth(dataset.last(), gt_path)
